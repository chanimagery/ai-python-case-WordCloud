...
with open(os.path.join(cur_path,'择天记.txt'),'r') as fp:
    text=fp.read()
    text=chinese_jieba(text);

    #print(text)
    wordcloud = WordCloud(background_color='black',font_path="FZLTXIHK.TTF",stopwords=stopwords).generate(text)  # 指定中文 去除噪声词 字体文件需要与py 文件处于同一目录
